...
export class CostFetchTaskScheduler {
    ...
    async initialize() {
        ...
        this.logger.info(`Configuring cost data fetch task with schedule "${schedule}" and initial delay of ${initialDelayMinutes} minutes`);
        ...
        this.logger.info(`Scheduled cost data fetch task (runs with schedule: ${schedule}, initial delay: ${initialDelayMinutes} minutes)`);
    }
}